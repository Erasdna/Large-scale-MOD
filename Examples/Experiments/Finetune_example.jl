...
using .LSMOD
"""
    Run a simple experiment testing:
        1) The number of solutions M in the history matrix
        2) The size of the reduced basis the we keep (m/M)
    
    Options:
        1) Method: Nystrom, RQR, RSVD 
        2) Timestep: 10e_3 or 10e_5
        3) Whether or not we should do row sampling: row or nothing
        4) Problem size (per dimension) 
        5) Seed
"""
